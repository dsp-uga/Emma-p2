...
#categoryConverter = IndexToString(inputCol="prediction", outputCol="predCategory", labels="indexed")
pipeline = Pipeline(stages=[categoryIndexer, tokenizer, hashingTF, nb])

model = pipeline.fit(df_train_original)
pr = model.transform(df_train_original)

print(pr)
pr.select('text','category','prediction','label').show()
'''
evaluator = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction", metricName="f1")
metric = evaluator.evaluate(pr)


indexer = StringIndexer(inputCol="class_label", outputCol="label")
tokenizer = Tokenizer(inputCol="text", outputCol="words")
...
nb = NaiveBayes(smoothing=1.0, modelType="multinomial")
#categoryConverter = IndexToString(inputCol="prediction", outputCol="predCategory", labels=indexer.labels)
pipeline = Pipeline(stages=[indexer,tokenizer,hashingTF,nb])

model = pipeline.fit(df_train_original)
print("NaiveBayes Trained.")
pr = model.transform(df_train_original)
print("NaiveBayes Tested")
#convertor=IndextoString(inputCol="prediction", outputCol="predictedLabel", labels=indexer.labels)
print(pr)
#pr.select('class_label','rawPrediction','probability','prediction').show()
pr.select('prediction','label','class_label').show()

pr['fianlout']=pr['classlabel'](if df['label'].equals(df['col2']))
print(pr)

df['outputfinal'] = np.where((pr['label'] == pr['prediction'])
                     , pr['class_label'], np.nan)
