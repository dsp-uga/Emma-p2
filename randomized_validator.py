...
def intialize_model():
	...
	model_0  = LinearSVC(maxIter=500, regParam=0.1,labelCol="one")
	model_1  = LinearSVC(maxIter=500, regParam=0.1,labelCol="two")
	model_2  = LinearSVC(maxIter=500, regParam=0.1,labelCol="three")
...
def boosting(df_tfidf_train,model,labelCol,limit_size):
		accuracy = [0 for i in range(0,8)]
		print("performing boosting")
		training_sample=df_tfidf_train.sample(True,split_train,seed=42).filter(df_tfidf_train.class_label==labelCol).limit(limit_size)
		training_sample=training_sample.union(df_tfidf_train.randomSplit([0.7,0.3])[0].filter(df_tfidf_train.class_label!=labelCol).limit(limit_size))
		temp = model.fit(training_sample)
		if labelCol=="one" or labelCol == "two" or labelCol=="three":
			model  = LinearSVC(maxIter=500, regParam=0.1,labelCol=labelCol)
		else:
			model = LogisticRegression(maxIter=500, regParam=0.3, elasticNetParam=0.8,labelCol=labelCol)

		model.coefficients = temp.coefficients
		model.intercept = temp.intercept
		print("iteration 2")
		training_sample=df_tfidf_train.randomSplit([0.7,0.3])[0].filter(df_tfidf_train.class_label==labelCol).limit(limit_size)
		training_sample=training_sample.union(df_tfidf_train.randomSplit([0.7,0.3])[0].filter(df_tfidf_train.class_label!=labelCol).limit(limit_size))
		temp = model.fit(training_sample)

		if labelCol=="one" or labelCol == "two" or labelCol=="three":
			model  = LinearSVC(maxIter=100, regParam=0.1,labelCol=labelCol)
		else:
			model = LogisticRegression(maxIter=500, regParam=0.3, elasticNetParam=0.8,labelCol=labelCol)
...
def prediction_and_label(model,dataset,class_label,validation=False):
	...
	if validation:
		...
		print("------------------")
		print("Class: "+ class_label + "f1-score "+str(accuracy))
		print("------------------")
...
training = sqlContext.createDataFrame(rdd_train,schema=["text","class_label","key"])
...
print("Model 1")
model_list[0]= boosting(training,model_list[0],"one",30000)


model_list[1]= boosting(training,model_list[1],"two",30000)
print("Model 3")
model_list[2]= boosting(training,model_list[2],"three",30000)
print("Model 4")
model_list[3]= boosting(training,model_list[3],"four",20000)
print("Model 5")
model_list[4]= boosting(training,model_list[4],"five",20000)
print("Model 6")
model_list[5]= boosting(training,model_list[5],"six",20000)
print("Model 7")
model_list[6]= boosting(training,model_list[6],"seven",20000)
print("Model 8")
model_list[7]= boosting(training,model_list[7],"eight",20000)
print("Model 9")
model_list[8]= boosting(training,model_list[8],"nine",20000)
print("Model 2")

testing_1 = testing.randomSplit([0.9,0.1])[0]


testing_2 = testing.randomSplit([0.9,0.1])[0]

testing_3 = testing.randomSplit([0.9,0.1])[0]

testing_4 = testing.randomSplit([0.9,0.1])[0]

testing_5 = testing.randomSplit([0.9,0.1])[0]
